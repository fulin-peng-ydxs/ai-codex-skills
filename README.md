...
| `git-commit-msg` | 基于已确认的 Git 改动范围生成规范化提交消息，默认使用 `Agent` 项目标识，并按单行模板或完整模板输出提交文案；确认节点必须依赖当前环境自带的交互式提问工具。 |
| `git-push` | 在用户明确同意的前提下完成 Git 提交与推送，覆盖文件范围确认、提交文案确认、staged 清单校验和 push 前复核；确认节点必须依赖当前环境自带的交互式提问工具。 |
...
14. 分析上市公司质量、估值、风险、评分或建仓情景时，可使用 `five-dim-stock-analysis`；完整分析必须先核对数据质量和最新官方报告，季报增量跟踪必须有可用的完整分析基线，证据不足的评分项和价格区间不得强行补值。
15. 涉及 Codex 自动化的创建、精简或排障时，优先使用 `automation-setup-assistant`；如果自动化依赖其他技能或项目文档，提示词应只补充固定目标、差异化约束和外部副作用边界，不要重复整段技能或项目规则。
16. 需要按 Git 提交范围审查近期 AI 自动开发结果时，使用 `agent-auto-commit-audit`；它负责串联代码审查、业务闭环核查、页面审查、必要修复和文档同步，不替代单独的技能开发或常规提交消息生成。
17. 涉及 Git 提交文案生成时，使用 `git-commit-msg`；它只负责基于已确认范围输出提交消息，不负责执行 `git add`、`git commit` 或 `git push`，且确认步骤必须使用当前环境自带的交互式提问工具，工具不可用时应中止而不是退回纯文本提问。
18. 需要实际执行 Git 提交与推送时，使用 `git-push`；它负责在当前环境下完成文件范围确认、提交文案确认、staged 清单复核和最终推送，不替代单独的提交消息生成规范；若当前环境缺少可用的交互式提问工具，也必须中止流程而不是改用普通文字确认。
19. 需要把 Codex 的全局 skills 或协作说明对齐到 Claude Code、Kimi Code 时，使用 `sync-codex-config`；它必须调用随技能提供的 `scripts/sync_codex_config.py`，默认先执行 `--dry-run` 预览，再按用户授权执行正式同步；Claude 目标文档写入 `CLAUDE.md`，Kimi 目标文档写入 `AGENTS.md`，不会删除目标端独有技能、文件或其他安装配置，并且要显式排除所有 MCP 相关技能。
