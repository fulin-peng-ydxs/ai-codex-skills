...
| `requirement-doc-generator` | 基于已确认的需求闭环和项目现状生成正式 `requirement.md`；新增页面需配套 `prototypes/` 下的 HTML 原型，建议实现和优化项必须带处理状态，且原型需明确区分正式功能、禁用/暂不实现控件、说明批注和原型演示控件，落地前先向用户确认。 |
...
8. `auto-plan-dev` 读取这些原型作为计划拆解、页面实现映射和验证依据，不得跳过原型直接按泛化文字拆任务；执行过程中更新计划时，必须同步补充任务的实际落地情况、各类测试/验证的具体完成情况，以及可选增强是否纳入或跳过的开发状态，不能只改状态。
...
- 若需求闭环、需求文档或开发计划技能新增了稳定状态枚举、范围映射、原型约束或是否纳入的约束，也要同步检查 README 是否已写清这些维护规则。
