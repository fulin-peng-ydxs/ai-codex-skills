...
| `git-commit-msg` | 基于已确认的 Git 改动范围生成规范化提交消息，默认使用 `Agent` 项目标识，并按单行模板或完整模板输出提交文案。 |
| `git-push` | 在用户明确同意的前提下完成 Git 提交与推送，覆盖文件范围确认、提交文案确认、staged 清单校验和 push 前复核。 |
...
16. 涉及 Git 提交文案生成时，使用 `git-commit-msg`；它只负责基于已确认范围输出提交消息，不负责执行 `git add`、`git commit` 或 `git push`。
17. 需要实际执行 Git 提交与推送时，使用 `git-push`；它负责在当前环境下完成文件范围确认、提交文案确认、staged 清单复核和最终推送，不替代单独的提交消息生成规范。
...
- 若 Git 提交流程类技能新增了稳定的确认节点、staged 校验或 push 安全约束，也要同步检查 README 是否已准确反映职责分工。
