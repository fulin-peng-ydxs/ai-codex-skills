...
生成或更新项目 `DESIGN.md` 时，优先使用本模板。占位内容必须替换为项目真实信息；无法从项目中确认的内容，不要伪造，应写入 `Known Gaps` 或先向用户确认。
...
说明项目界面的整体视觉主题、使用场景、信息密度、情绪基调和界面哲学。必须基于项目现有前端归纳，不写品牌化空话。

应包含 `Key Characteristics`，用短句列出后续 AI 必须保持的关键特征。

## 2. Color Palette & Roles

说明主色、辅助色、背景色、表面色、文本色、边框色和状态色的语义角色。若项目已有 CSS 变量或主题 token，应在这里说明来源。
...
提示内容必须基于本项目真实规范，不要引用外部品牌。

## 10. Known Gaps

记录当前项目缺少或无法从代码中可靠确认的设计规范。若无明显缺口，写“暂无已确认缺口”。
```
